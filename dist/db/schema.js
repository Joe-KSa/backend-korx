...
export const projectScores = sqliteTable("project_scores", {
    ...
    uniqueProjectMemberScore: unique("uq_project_scores_project_id_member_id").on(table.projectId, table.memberId),
}));
...
export const memberThemes = sqliteTable("member_themes", {
    memberId: integer("member_id").references(() => members.id, {
        onDelete: "cascade",
    }),
    themeId: integer("theme_id").references(() => profileThemes.id, {
        onDelete: "cascade",
    }),
    active: integer("active").notNull().default(0), // 1 = activado, 0 = desactivado
}, (table) => ({
    uniqueMemberTheme: unique("uq_member_themes_member_id_theme_id").on(table.memberId, table.themeId),
}));
// Badges
...
export const memberBadges = sqliteTable("member_badges", {
    memberId: integer("member_id").references(() => members.id, {
        onDelete: "cascade",
    }),
    ...
    uniqueMemberBadge: unique("uq_member_badges_member_id_badge_id").on(table.memberId, table.badgeId),
}));
// Moderation
// Moderators Table
export const moderators = sqliteTable("moderators", {
    ...
    uniqueModerator: unique("uq_moderators_user_id_role_id").on(table.userId, table.roleId),
}));
// Permissions Table
...
export const moderatorPermissions = sqliteTable("moderator_permissions", {
    ...
    uniqueModeratorPermission: unique("uq_moderator_permissions").on(table.userId, table.roleId, table.permissionId),
}));
